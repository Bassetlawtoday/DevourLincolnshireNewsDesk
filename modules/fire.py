...
from newsdesk.sources.fire_scraper import _parse_fire_publication_datetime
...
class FireIntelligenceWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master=None):
        ...
        self.title("Fire Intelligence - Devour Lincolnshire NewsDesk")
    ...
    def _open_source_manager(self):
        ...
        self.source_manager_window = SourceManagerWindow(
            self,
            module_profile="fire",
            on_refresh=self._refresh_Fire_news,
        )
    ...
    def _build_story_queue(self, parent):
        ...
        Fire-specific filtering and selection remain in this window, while
        the reusable component owns queue layout, rendering, counts and
        empty-state presentation.
        """

        queue_panel = ctk.CTkFrame(parent, fg_color="transparent")
        queue_panel.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
        ...
        self.story_queue = NewsDeskStoryQueue(
            queue_panel,
            empty_title="No Fire stories loaded.",
            empty_message=(
                "Select “Refresh FIRE NEWS” to collect the latest releases."
            ),
    ...
    def _build_filter_controls(self, parent):
        ...
        self.search_entry = ctk.CTkEntry(
            ...
            placeholder_text="Search incident, place or keyword...",
    ...
    def _filter_option_menu(self, parent, values):
        ...
        self._style_fire_option_menu(menu)
        return menu

    @staticmethod
    def _style_fire_option_menu(menu):
        menu.configure(
            fg_color=CARD_BG,
            button_color=ACCENT,
    ...
    def _build_editorial_review(self, parent):
        ...
        ctk.CTkLabel(
            self.review_placeholder,
            text=(
                "Choose a Fire story from the queue to view its summary,\n"
    ...
    def _build_status_bar(self):
        ...
        ctk.CTkLabel(
            status_bar,
            text="Fire Intelligence • Live NewsDesk",
            font=("Arial", 11),
            text_color=TEXT_MUTED,
        ).grid(
            row=0,
    ...
    # ------------------------------------------------------------------
    # Fire collection and queue
    # ------------------------------------------------------------------

    def _refresh_Fire_news(self):
        ...
        worker = threading.Thread(
            target=self._collect_Fire_news,
            daemon=True,
        )
        worker.start()

    def _collect_Fire_news(self):
        try:
            service = FireCollectionService()
    ...
    def _collection_complete(self, stories, results, errors, notify_dashboard=True):
        ...
        if errors:
            ...
            self.status_label.configure(
                text=f"Status: Loaded {total} Fire stories."
            )
    ...
    def load_stories(self, stories, publish_results=None, errors=None):
        ...
        self.status_label.configure(
            text=f"Status: Loaded {len(self.stories)} Fire stories from NewsDesk Pro Home."
        )
    ...
    def _collection_failed(self, error_message):
        ...
        self.collection_progress.grid_remove()
        self.status_label.configure(
            text="Status: FIRE NEWS collection failed."
        )

        messagebox.showerror(
            "Fire collection failed",
            "The latest Fire releases could not be collected.\n\n"
            f"{error_message}",
            parent=self,
        )
    ...
    def _refresh_visible_queue(self, selected_filter=None):
        ...
        self.status_label.configure(
            text=(
                ...
                f"{len(self.stories)} Fire stories."
            )
        )
    ...
    def _sort_visible_stories(self, stories, selected_sort):
        ...
        if selected_sort == "Newest first":
            ...
            visible.sort(key=self._published_sort_value)
        elif selected_sort == "Title A–Z":
            visible.sort(key=lambda story: str(story.title or "").casefold())
    ...
    def _published_sort_value(story):
        parsed = _parse_fire_publication_datetime(getattr(story, "published", ""))
    ...
    def _populate_story_queue(self, stories):
        """
        Convert Fire Story objects into the shared queue's mapping format.
        """
        ...
        self.story_queue.set_stories(
            self._queue_story_mapping(story) for story in visible
        )
        self.queue_count_label.configure(
            text=f"{len(visible)} of {len(self.stories)} stories"
        )
    ...
    def _queue_story_mapping(self, story):
        """
        Adapt one Fire Story for the reusable NewsDeskStoryQueue.
        """
    ...
    def _handle_queue_story_selected(self, queue_story):
        """
        Resolve a shared-queue mapping back to its Fire Story object.
        """
    ...
    def _save_selected_image(self):
        ...
        title = self._safe_filename(
            getattr(self.selected_story, "title", "Fire-story-image")
        )
    ...
    def _safe_filename(value):
        ...
        return cleaned[:120] or "Fire-story-image"
    ...
    def _export_selected_story_pack(self):
        ...
        safe_title = self._safe_filename(
            getattr(story, "title", "") or "Fire-story"
        )

        destination = filedialog.asksaveasfilename(
            parent=self,
            title="Export Fire story pack",
        ...
        try:
            ...
            messagebox.showinfo(
                "Story Pack exported",
                (
                    "The Fire Story Pack has been created successfully.\n\n"
                    f"{destination}"
                ),
                parent=self,
            )
    ...
    def _run_action(self, action_name):
        ...
        if action_name == "ADD TO NEWSLETTER":
            open_newsletter_action(
                self, self.selected_story, module_key="fire",
                newsletter_copy=str(
        ...
        if result is None:
            messagebox.showwarning(
                ...
                (
                    ...
                    "Refresh the FIRE NEWS feed and try again."
                ),
                parent=self,
            )
            return
    ...
    def _show_output_window(self, title, content):
        ...
        window.title(f"{title.title()} - FireColleFire IntelligencectionService")
...
def open_fire(master=None):
    """Open the Fire Intelligence workspace."""

    window = FireIntelligenceWindow(master)
    return window
...
if __name__ == "__main__":
    ...
    window = FireIntelligenceWindow(app)
